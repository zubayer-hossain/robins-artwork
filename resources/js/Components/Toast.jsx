...
function ToastItem({ toast, onRemove }) {
    ...
    const getToastStyles = () => {
        const baseStyles = "relative flex items-start p-3 sm:p-4 rounded-lg shadow-lg backdrop-blur-sm border transition-all duration-300 ease-out transform";
    ...
    return (
        <div className={getToastStyles()}>
            ...
            <div className="ml-2 sm:ml-3 flex-1">
                {toast.title && (
                    <p className="text-xs sm:text-sm font-semibold mb-0.5 sm:mb-1">{toast.title}</p>
                )}
                <p className="text-xs sm:text-sm">{toast.message}</p>
            </div>
            <button
                onClick={handleRemove}
                className="ml-2 sm:ml-4 flex-shrink-0 text-gray-400 hover:text-gray-600 transition-colors duration-200 p-1"
            >
...
export function ToastContainer({ toasts, removeToast }) {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 space-y-2 max-w-xs sm:max-w-sm w-full pointer-events-none px-4 sm:px-0">
            {toasts.map((toast) => (
